...
```{r}
#Based on the previous map, we want to narrow in on the areas that illustrate higher levels of particulate matter - specifically areas illustrating higher percentage of minority populations.
...
```{r}
# Saving a map for README

tm_shape(los_angeles, bbox = c(-118.38,33.8,-117.7,34.2)) +
  tm_polygons(
    fill = "PEOPCOLORPCT",
    fill.scale = tm_scale_intervals(values = "Blues", style = "quantile"),
    fill.legend = tm_legend(title = "% Minority Population")
...
tmap_save(la_county_min, here::here("figs","la_county_part.jpg"))
```
